...
with st.form("auditoria_form"):
    url_maps = st.text_input("1. URL de Google Maps de la ficha (Obligatorio):")
    url_web = st.text_input("2. URL de la página Web (Opcional - Déjalo en blanco si no tiene):")
    enviar = st.form_submit_button("Auditar Negocio")

if enviar:
    if not url_maps:
        st.warning("Por favor, introduce al menos la URL de Google Maps para poder auditar.")
    else:
        with st.spinner("Rastreando y calculando baremos en tiempo real..."):
            
            # Valores por defecto para la Ficha
            score_ficha = 100
            score_servicios = 50  
            score_reputacion = 80
            score_visuales = 50
            ...
            # Intentar rascar info de Google Maps
            ...
            # Si el usuario ha puesto Web, la analizamos
            if url_web.strip():
                score_web = 20
                tiene_schema = False
                tiene_sitemap = False
                ...
                # Nota final contando con la web
                nota_final = int(
                    (score_ficha * 0.20) + 
                ...
                brechas_web = f"""- Schema LocalBusiness: {"✅ Detectado" if tiene_schema else "❌ FALTANTE (Prioridad Alta)"}
...
- Sitemap XML en Robots: {"✅ Indexado" if tiene_sitemap else "❌ NO DETECTADO"}"""
                ...
                # Si NO tiene web, el Score Web es 0 y recalculamos los pesos sin que le penalice en el resto
                score_web = 0
                nota_final = int(
                    (score_ficha * 0.25) + 
                    (score_servicios * 0.25) + 
                    (score_reputacion * 0.30) + 
                    (score_visuales * 0.20)
                )
                brechas_web = "❌ EL NEGOCIO NO TIENE WEB (Oportunidad brutal de venta)"
